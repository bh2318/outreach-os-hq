// Build a single self-contained HTML mock site, upload to mock-sites bucket,
// and create a mock_sites DB row with the public URL.
...
const NICHE_THEME: Record<string, { bg: string; accent: string; headline: (city: string) => string; services: { icon: string; name: string; desc: string }[] }> = {
  plumber: {
    bg: "#0b2a4a",
    accent: "#3aa0ff",
...
function buildHtml(lead: Lead): string {
  const niche = (lead.niche || "service").toLowerCase();
  const theme = NICHE_THEME[niche] ?? NICHE_THEME.plumber;
  const city = lead.city || "your area";
  const name = lead.business_name;
  const reviewCount = lead.site_audit_json?.review_count ?? 0;
  const rating = lead.site_audit_json?.rating ?? 5.0;
  const phone = lead.phone || fakePhone(name);
...
.hero{background:${theme.bg};color:#fff;padding:80px 24px 96px;text-align:center;position:relative;overflow:hidden}
...
<body>
<div class="preview-banner">This is a free preview created for ${escapeHtml(name)} by Brad Hemminger — reply to this email to get started.</div>

<header class="hero">
  <div class="hero-inner">
    <h1>${escapeHtml(name)}</h1>
    <div class="tagline">${escapeHtml(theme.headline(city))}</div>
    <div class="social"><span class="stars">${stars}</span> <span><strong>${rating}</strong> from ${reviewCount} Google reviews</span></div>
    <div class="ctas">
...
<section class="section">
  ...
  <div class="grid-3">
    ${theme.services.map((s) => `
      <div class="card"><div class="icon">${s.icon}</div><h3>${s.name}</h3><p>${s.desc}</p></div>
    `).join("")}
  </div>
</section>
...
<section class="section about">
  ...
  <p>We're a locally owned ${escapeHtml(niche)} based right here in ${escapeHtml(city)}. We've earned our reputation one customer at a time by showing up when we say we will, doing the job right the first time, and treating every home like it's our own. When you call us, you're calling neighbors — not a faceless chain.</p>
</section>
  ...
  <div class="inner">
    ...
    <div class="contact-grid">
      <div class="contact-info">
        ...
        <p>📍 ${escapeHtml(city)}, ${escapeHtml(lead.state || "")}</p>
...
<footer>
  <div><span class="name">${escapeHtml(name)}</span> &nbsp;·&nbsp; ${escapeHtml(city)}, ${escapeHtml(lead.state || "")} &nbsp;·&nbsp; ${escapeHtml(phone)}</div>
  <div style="margin-top:8px">© 2026 ${escapeHtml(name)}. All rights reserved.</div>
</footer>
</body></html>`;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: lead, error: leadErr } = await supabase
      .from("leads")
      .select("id,business_name,owner_name,city,state,county,niche,phone,site_audit_json")
      .eq("id", leadId)
      .single();
    if (leadErr || !lead) throw new Error(`Lead not found: ${leadErr?.message}`);

    const html = buildHtml(lead as Lead);
    const fileName = `${slugify(lead.business_name)}-preview.html`;
    ...
    const { data: mockRow, error: mockErr } = await supabase
      ...
      .select("id")
      .single();
    if (mockErr) throw new Error(`mock_sites insert failed: ${mockErr.message}`);

    return new Response(
      JSON.stringify({ success: true, url, mockSiteId: mockRow.id, fileName }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (err) {
    console.error("generate-mock-site error:", err);
    return new Response(
